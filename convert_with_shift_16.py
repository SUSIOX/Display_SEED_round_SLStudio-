...
def convert_with_shift(img_path, var_name, output_path, shift_y):
    ...
    for y in range(height):
        row = []
        # Celkový posun 16px nahoru (čteme y+16)
        read_y = y + shift_y
        if read_y >= height:
            read_y = height - 1
        if read_y < 0:
...
# Celkový posun 16px nahoru
convert_with_shift('src/images/ocix02_open_240_scaled.png', 'ui_img_ocix02_mono_png', 'src/images/ui_img_ocix02_mono_png.c', 16)
convert_with_shift('src/images/ocix01_closed_240_scaled.png', 'ui_img_ocix01_c_png', 'src/images/ui_img_ocix01_c_png.c', 16)
